...
This is the file made in class with some tweaks
Please see read Main.cs for more information
...
public class SteeringBehaviours : MonoBehaviour {
	...
	void Start()
	{
		...
		seekTargetPos = target.transform.position;
	}
	
	// Update is called once per frame
	void Update () 
	{
		if(target!=null)	// Protection in case the target was destroyed
			seekTargetPos = target.transform.position;
		else
		{
			// Lets have some fun. If we destroy the leader
			// make the girl the default target :)
			
			target = GameObject.Find("Girl");
		}
			
		ForceIntegrator();   // The core !
	}
	...
	void ForceIntegrator()
	{
		...
		//if(!target.gameObject.CompareTag("Leader"))
	...
	bool AccumulateForce(ref Vector3 runningTotal, Vector3 force) //This method takes the force from the above method (eg. force = Seek(target.transform.position)) and passes it down to be checked against the remaining force available. 
	{
		...
		// Adjust Y coordinate so the ants dont fly (only if its not the leader)
		/*
		if(!target.gameObject.CompareTag("Leader"))
		{
			//target.transform.position = new Vector3(target.transform.position.x, 3f, target.transform.position.z);
			//transform.position = new Vector3(transform.position.x, 3f, transform.position.z);
			//runningTotal = new Vector3(runningTotal.x, 2f, runningTotal.z);
		}
		*/
	...
	Vector3 Seek(Vector3 target) //This is the Seek behaviour! It returns a Vector3 to the force, which is where we steer this agent towards
	{
		...
		// Avoid the enemies following the player go underneath the floor
		//if(this.target.CompareTag("Player"))
		//	target = new Vector3(target.x, 20f, target.z);
	...
	Vector3 Pursue(GameObject target) //Pursue is useful for calculating a future interception position of a target. It is used in conjunction with Seek!
	{
		...
		// The player doesn't have a Steering behaviour script so I get the velocity from the Rigidbody Component
		if(target.CompareTag("Player"))
		{
			desPos = target.transform.position+(lookAhead * target.GetComponent<Rigidbody>().velocity);
		}
		else
			desPos = target.transform.position+(lookAhead * target.GetComponent<SteeringBehaviours>().velocity);
		
		return Seek (desPos); //we return our vector to Seek, which then runs the normal Seek code
	}
	
	// Well, I don't use this one in my project but I will leave for future references
